"""
Script pour dupliquer les templates système pour tous les tenants existants.

Ce script crée les copies personnalisables des templates
SYSTEM_CONSOLIDATED et SYSTEM_INDIVIDUAL pour chaque client existant.

Usage (depuis le dossier backend):
    python duplicate_templates_for_existing_tenants.py
"""
...
def duplicate_templates_for_all_tenants():
    """Duplique les templates système pour tous les tenants existants."""

    print("=" * 70)
    print("DUPLICATION DES TEMPLATES POUR TOUS LES TENANTS")
    print("=" * 70)
    print()

    db = SessionLocal()
    try:
        # 1. Récupérer les templates système à dupliquer
        templates_query = text("""
            ...
            WHERE code IN ('SYSTEM_CONSOLIDATED', 'SYSTEM_INDIVIDUAL')
              AND is_system = true
        ...
        if not system_templates:
            print("[ERROR] Aucun template systeme trouve!")
            print("        Executez d'abord: python insert_consolidated_template.py")
            return

        print(f"[OK] {len(system_templates)} templates systeme trouves:")
        ...
        for tenant in tenants:
            ...
            # Vérifier les templates existants pour ce tenant
            existing_query = text("""
            ...
            for template in system_templates:
                ...
                original_code = template[3]  # code (ex: SYSTEM_CONSOLIDATED)
                base_code = original_code.replace('SYSTEM_', '')  # CONSOLIDATED
                template_category = template[14] if len(template) > 14 else 'audit'  # Catégorie (audit par défaut)
                ...
                new_id = uuid4()
                new_name = f"{template[1]} : {tenant_name}"  # Ex: "Rapport Consolidé Écosystème : AHAJRI"
                ...
                db.execute(insert_query, {
                    "id": new_id,
                    "tenant_id": tenant_id,
                    "parent_template_id": parent_template_id,  # Lien vers le template maître
                    "name": new_name,
                    "description": template[2],
                    "code": tenant_code,
                    "template_type": template[4],
                    "template_category": template_category,  # Hériter de la catégorie du parent
        ...
        print("=" * 70)
        print(f"[SUCCESS] {total_created} templates crees au total")
        print("=" * 70)
        print()
...
if __name__ == "__main__":
    duplicate_templates_for_all_tenants()
