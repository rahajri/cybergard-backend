...
class EbiosReportService:
    ...
    async def _collect_actions(self, project_id: UUID) -> List[Dict]:
        ...
        Priorité:
        1. Si le projet a des actions publiées (actions_published_at IS NOT NULL),
           récupérer depuis published_action avec leur statut actuel
        2. Sinon, récupérer depuis ai_raw_output du workshop AT5 (actions non publiées)
        """
        # 1. Vérifier si le projet a des actions publiées
        project_check = text("""
            SELECT actions_published_at
            FROM risk_project
            WHERE id = CAST(:project_id AS uuid)
        """)
        project_result = self.db.execute(project_check, {"project_id": str(project_id)})
        project_row = project_result.fetchone()

        if project_row and project_row.actions_published_at:
            # Actions publiées - récupérer depuis published_action
            published_query = text("""
                SELECT
                    code_action,
                    title,
                    description,
            ...
            """)
            result = self.db.execute(published_query, {"project_id": str(project_id)})
            ...
            if rows:
                ...
                for row in rows:
                    # Extraire les données AI stockées dans ai_justifications
                    ai_data = row.ai_justifications or {}
                    if isinstance(ai_data, str):
                        try:
                            ai_data = json.loads(ai_data)
                    ...
                    action = {
                        ...
                        'objectif': row.objective or '',
                        'priorite': row.severity or 'Moyenne',
                        'statut': status_map.get(row.status, 'En attente'),
                        'responsable_suggere': row.suggested_role or 'Non défini',
                        'echeance': row.due_date.strftime('%d/%m/%Y') if row.due_date else f"{row.recommended_due_days or 90} jours",
                        # Données enrichies depuis ai_justifications
                        'justification': ai_data.get('justification', ''),
                        'scenarios_couverts': ai_data.get('scenarios_couverts', []),
                        'sources_couvertes': ai_data.get('sources_couvertes', []),
                        'biens_supports': ai_data.get('biens_supports', []),
                        'references_normatives': ai_data.get('references_normatives', []),
                        'risque_initial': ai_data.get('risque_initial'),
                        'risque_cible': ai_data.get('risque_cible'),
                        'effort': ai_data.get('effort', ''),
                        'cout_estime': ai_data.get('cout_estime', ''),
                        # scenario_ref pour compatibilité template
                        'scenario_ref': ', '.join(ai_data.get('scenarios_couverts', [])[:3]) or '-'
                    }
                    actions.append(action)

                logger.info(f"📋 {len(actions)} actions publiées récupérées pour rapport EBIOS")
                return actions

        # 2. Fallback: actions non publiées depuis ai_raw_output du workshop AT5
        workshop_query = text("""
            SELECT ai_raw_output FROM risk_workshop
            WHERE project_id = CAST(:project_id AS uuid)
              AND type = 'AT5'
        ...
        if row and row.ai_raw_output:
            try:
                ...
                # Normaliser les champs pour le template de rapport
                for action in actions:
                    # Mapper action_id vers code_action pour le template
                    if 'action_id' in action and 'code_action' not in action:
                        action['code_action'] = action['action_id']
                    # Mapper statut_initial vers statut (valeur par défaut: "En attente")
                    if 'statut' not in action:
                        action['statut'] = action.get('statut_initial', 'En attente')
                    # Mapper delai_recommande vers echeance
                    if 'echeance' not in action:
                        action['echeance'] = action.get('delai_recommande', '-')
                    # Créer scenario_ref à partir de scenarios_couverts
                    if 'scenario_ref' not in action and 'scenarios_couverts' in action:
                        scenarios = action.get('scenarios_couverts', [])
                        action['scenario_ref'] = ', '.join(scenarios[:3]) if scenarios else '-'

                logger.info(f"📋 {len(actions)} actions non publiées récupérées pour rapport EBIOS")
                return actions
            except (json.JSONDecodeError, AttributeError):
                pass
...
Génère 2-3 phrases d'analyse."""
    ...
    def get_ebios_template(self, report_scope: str = "consolidated"):
        ...
        1. Template tenant spécifique (tenant_id = :tenant_id)
        2. Template système (tenant_id IS NULL, is_system = true)
        ...
        """
        # Mapper le scope vers le code du template
        if report_scope == "individual":
            scope_code = "INDIVIDUAL"
        else:
            scope_code = "CONSOLIDATED"

        # Étape 1: Chercher le template du tenant
        tenant_query = text("""
            ...
            WHERE tenant_id = CAST(:tenant_id AS uuid)
              ...
              AND code ILIKE :scope_pattern
            ORDER BY
                is_default DESC,
                created_at DESC
            LIMIT 1
        """)
        result = self.db.execute(tenant_query, {
            "tenant_id": str(self.tenant_id),
            "scope_pattern": f"%{scope_code}%"
        })
        row = result.fetchone()

        # Étape 2: Si pas de template tenant, chercher le template système
        if not row:
            logger.info(f"📋 Pas de template EBIOS tenant pour scope '{report_scope}', recherche système...")
            system_query = text("""
                SELECT id, name, code, color_scheme, fonts, structure, custom_css,
                       default_logo, custom_logo
                FROM report_template
                ...
                LIMIT 1
            """)
            result = self.db.execute(system_query, {
                "scope_pattern": f"%{scope_code}%"
            })
            row = result.fetchone()
        ...
        if row:
            ...
            logger.warning("⚠️ Aucun template EBIOS trouvé (ni tenant ni système)")
            return None, None
