...
class ScanActionItemResponse(BaseModel):
    ...
    suggested_role: Optional[str] = None
    assigned_user_id: Optional[str] = None
    assigned_user_name: Optional[str] = None  # Nom de l'utilisateur assigné (pour affichage)
...
async def get_scan_action_plan(
    ...
    # Récupérer les items avec le nom de l'utilisateur assigné
    items_query = text("""
        SELECT sapi.*,
               em.first_name as member_first_name,
               em.last_name as member_last_name
        FROM scan_action_plan_item sapi
        LEFT JOIN entity_member em ON em.id = sapi.assigned_user_id
        WHERE sapi.scan_action_plan_id = CAST(:plan_id AS uuid)
        ORDER BY sapi.order_index
    """)
    ...
    for row in items_rows:
        ...
        if row.assigned_user_id:
            if hasattr(row, 'member_first_name') and row.member_first_name:
                assigned_user_name = f"{row.member_first_name} {row.member_last_name or ''}".strip()
        ...
        items.append(ScanActionItemResponse(
            ...
            suggested_role=row.suggested_role if hasattr(row, 'suggested_role') else None,
...
async def update_scan_action_plan_item(
    ...
    if request.entity_id is not None:
        if request.entity_id:
            ...
            # entity_id vide = mode interne, effacer entity_id et entity_name
            updates.append("entity_id = NULL")
            updates.append("entity_name = :entity_name")
            params["entity_name"] = "Interne"
    elif request.entity_name is not None:
        # entity_name fourni sans entity_id (mode interne)
        updates.append("entity_name = :entity_name")
        params["entity_name"] = request.entity_name

    # Gestion de assigned_user_id
    if request.assigned_user_id is not None:
        if request.assigned_user_id:
            updates.append("assigned_user_id = CAST(:assigned_user_id AS uuid)")
            params["assigned_user_id"] = request.assigned_user_id
        else:
            updates.append("assigned_user_id = NULL")

    # Gestion de suggested_role
    if request.suggested_role is not None:
        updates.append("suggested_role = :suggested_role")
        params["suggested_role"] = request.suggested_role if request.suggested_role else None
    ...
    if request.recommended_due_days is not None:
        updates.append("recommended_due_days = :recommended_due_days")
        params["recommended_due_days"] = request.recommended_due_days
    ...
    logger.info(f"✅ Item {item_id} mis à jour avec succès - entity_id: {request.entity_id}, entity_name: {request.entity_name}, assigned_user_id: {request.assigned_user_id}")
