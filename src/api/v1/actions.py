...
class PublishedActionResponse(BaseModel):
    ...
    assignment_type: Optional[str] = None  # 'internal' ou 'external' - détermine la table source (users vs entity_member)
    assigned_entity_id: Optional[UUID] = None  # ID de l'entité (pour mode externe)
...
class AssignmentInfo:
    """Informations complètes sur l'assignation d'une action."""
    def __init__(self, user_name: Optional[str] = None, assignment_type: Optional[str] = None, entity_id: Optional[UUID] = None):
        self.user_name = user_name
        self.assignment_type = assignment_type  # 'internal' ou 'external'
        self.entity_id = entity_id  # ID de l'entité pour mode externe


def fetch_assignment_info(db: Session, assigned_user_id: Optional[UUID], entity_id: Optional[UUID] = None) -> AssignmentInfo:
    """
    Récupère les informations complètes d'assignation.

    Stratégie de recherche:
    1. Si entity_id est fourni → mode externe, chercher dans entity_member
    2. Sinon, chercher d'abord dans entity_member (audités externes)
    3. Si non trouvé, chercher dans users (utilisateurs internes)

    Args:
        db: Session de base de données
        assigned_user_id: UUID de l'utilisateur assigné
        entity_id: UUID de l'entité (indique mode externe si présent)

    Returns:
        AssignmentInfo avec user_name, assignment_type et entity_id
    """
    logger.info(f"🔍 fetch_assignment_info: assigned_user_id={assigned_user_id}, entity_id={entity_id}")

    if not assigned_user_id:
        logger.info("🔍 No assigned_user_id, returning empty AssignmentInfo")
        return AssignmentInfo()

    # Si entity_id est fourni, c'est forcément mode externe
    if entity_id:
        logger.info(f"🔍 entity_id provided, searching in entity_member...")
        query = text("""
            SELECT CONCAT(first_name, ' ', last_name) as full_name, entity_id
            FROM entity_member
        ...
        row = result.fetchone()
        if row and row[0]:
            logger.info(f"✅ Found in entity_member with entity_id: user_name={row[0]}, entity_id={row[1]}")
            return AssignmentInfo(
                user_name=row[0],
                assignment_type='external',
                entity_id=row[1] if row[1] else entity_id
            )
        else:
            logger.info(f"❌ Not found in entity_member with entity_id hint")

    # 1. Chercher d'abord dans entity_member (audités externes)
    logger.info(f"🔍 Searching in entity_member (no entity_id hint)...")
    query = text("""
        SELECT CONCAT(first_name, ' ', last_name) as full_name, entity_id
        FROM entity_member
        WHERE id = CAST(:user_id AS uuid)
    ...
    row = result.fetchone()
    if row and row[0]:
        logger.info(f"✅ Found in entity_member: user_name={row[0]}, entity_id={row[1]}, returning EXTERNAL")
        return AssignmentInfo(
            user_name=row[0],
            assignment_type='external',
            entity_id=row[1]
    ...
    logger.info(f"🔍 Not in entity_member, searching in users...")
    query = text("""
    ...
    row = result.fetchone()
    if row and row[0]:
        logger.info(f"✅ Found in users: user_name={row[0]}, returning INTERNAL")
        return AssignmentInfo(
            user_name=row[0],
    ...
    logger.warning(f"⚠️ User {assigned_user_id} not found in any table!")
    return AssignmentInfo()


def fetch_assigned_user_name(db: Session, assigned_user_id: Optional[UUID], campaign_id: Optional[UUID]) -> Optional[str]:
    """
    Récupère le nom de l'utilisateur assigné (fonction de compatibilité).

    Pour les nouvelles fonctionnalités, préférer fetch_assignment_info() qui retourne
    aussi le type d'assignation et l'ID de l'entité.
    """
    info = fetch_assignment_info(db, assigned_user_id)
...
async def get_actions(
    ...
    try:
        ...
        union_query = text(f"""
            WITH all_actions AS (
                ...
                SELECT
                    ...
                    false AS is_external_assignment
                ...
                -- Note: is_external_assignment permet de déterminer le type d'assignation
                SELECT
                    a.id,
                    a.code_action,
                    ...
                    CASE WHEN a.external_assignee_id IS NOT NULL THEN true ELSE false END AS is_external_assignment
                FROM action a
        ...
        for row in rows:
            ...
            source_type = row[31] if len(row) > 31 else 'published'
            is_external_assignment = row[32] if len(row) > 32 else False

            # DEBUG: Log pour les actions standalone
            if source_type == 'standalone':
                logger.info(f"📋 Standalone action {action_id}: is_external_assignment={is_external_assignment}, entity_id={entity_id_val}, assigned_user_id={assigned_user_id}")
            ...
            # Pour les actions standalone, utiliser is_external_assignment directement
            # car fetch_assignment_info peut ne pas trouver le bon type si l'utilisateur
            # existe dans les deux tables (users et entity_member)
            if source_type == 'standalone' and is_external_assignment:
                # Mode externe : on force le type et on utilise entity_id de l'action
                assignment_info = fetch_assignment_info(db, assigned_user_id, entity_id_val)
                assignment_info.assignment_type = 'external'
                assignment_info.entity_id = entity_id_val
                logger.info(f"🔧 Standalone action {action_id}: forced external assignment, entity_id={entity_id_val}")
            else:
                assignment_info = fetch_assignment_info(db, assigned_user_id, entity_id_val)

            # DEBUG: Log final assignment info
            if source_type == 'standalone':
                logger.info(f"📋 Standalone action {action_id} final: assignment_type={assignment_info.assignment_type}, entity_id={assignment_info.entity_id}")
            ...
            items.append(PublishedActionResponse(
                ...
                assigned_entity_id=assignment_info.entity_id,
...
async def get_action(
    ...
    try:
        ...
        if action:
            ...
            assignment_info = fetch_assignment_info(db, action.assigned_user_id, action.entity_id)
        ...
        # Déterminer le type d'assignation pour standalone
        if standalone_action.external_assignee_id:
            # Mode externe : external_assignee_id pointe vers entity_member
            assignment_info = fetch_assignment_info(db, standalone_action.external_assignee_id, standalone_action.entity_id)
        else:
            # Mode interne : assignee pointe vers users
            assignment_info = fetch_assignment_info(db, standalone_action.assignee, None)
...
async def update_action(
    ...
    try:
        ...
        if is_standalone:
            ...
            # Déterminer le type d'assignation : si external_assignee_id est utilisé, c'est externe
            standalone_entity_id = action.entity_id if action.external_assignee_id else None
            assignment_info = fetch_assignment_info(db, effective_assignee_id, standalone_entity_id)
...
async def create_standalone_action(
    ...
    try:
        ...
        if is_external_mode and action_data.assigned_user_id:
            ...
            assignment_info = fetch_assignment_info(db, response_assigned_user_id, action_data.entity_id)
            assignment_method = "manual"
        else:
            # Mode interne : utiliser assignee (users)
            response_assigned_user_id = new_action.assignee
            assignment_info = fetch_assignment_info(db, response_assigned_user_id, None)
